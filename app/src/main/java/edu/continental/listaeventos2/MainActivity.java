...
public class MainActivity extends AppCompatActivity implements Response.ErrorListener, Response.Listener<JSONObject>{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerEventos=(RecyclerView) findViewById(R.id.rvEventos);
    ...
    public void onResponse(JSONObject response) {
        Evento evento=null;
        JSONArray json=response.optJSONArray("List_Evento");
        try{
            for(int i=0;i<json.length();i++){
                evento=new Evento();
                JSONObject jsonObject=null;
                jsonObject=json.getJSONObject(i);
                evento.setNombre(jsonObject.optString("EVE_Nombres"));
                evento.setDescripcion(jsonObject.optString("EVE_Descripcion"));
                evento.setDetalle(jsonObject.optString("EVE_Detalles"));
                evento.setFotografia(jsonObject.optString("EVE_Fotografia"));
                evento.setFechahora(jsonObject.optString("EVE_FechaHora"));
